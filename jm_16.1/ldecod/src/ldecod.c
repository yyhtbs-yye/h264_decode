...
static void Configure(VideoParameters *p_Vid, InputParameters *p_Inp, int ac, char *av[])
{
  ...
  //strcpy(p_Inp->encoderLogName,"enc.log"); //! set default encoder log name
  ...
  while (CLcount < ac)
  {
    ...
    else if (0 == strncmp (av[CLcount], "-lp", 3))  
    {
      ...
      ++CLcount;
		...
		/***** XML_TRACE_BEGIN *****/
		else if (0 == strncmp (av[CLcount], "-xmltrace", 13))  
    {
			xml_set_trace_filename(av[CLcount+1]);
			CLcount += 2;
    }
		/****** XML_TRACE_END *****/
    else
...
static void init_conf(VideoParameters *p_Vid, InputParameters *p_Inp, char *config_filename)
{
  ...
  char linesInLog[FILE_NAME_SIZE];
	...
	/***** XML_TRACE_BEGIN *****/
	char xmltracefile[FILE_NAME_SIZE];
	int xmlloglevel = 4;
	/****** XML_TRACE_END *****/
  ...
  /***** XML_TRACE_BEGIN *****/
  conf_read_check (fscanf(fd,"%s",xmltracefile), 1);	// XML config file
  conf_read_check (fscanf(fd,"%*[^\n]"), 0);
  conf_read_check (fscanf(fd,"%d",&xmlloglevel), 1);	// XML log level
  conf_read_check (fscanf(fd,"%*[^\n]"), 0);
  ...
  if(!strcmp(p_Inp->encoderLogName,"")) p_Inp->bEncoderLog=0;
  else p_Inp->bEncoderLog=1;

  if(p_Inp->encoderLog != NULL) fclose(p_Inp->encoderLog);

	p_Inp->encoderLog = fopen(p_Inp->encoderLogName, "r");

	if(p_Inp->encoderLog == NULL) return -1;
  readyInLog=0;
  while(! readyInLog){
    fgets(linesInLog,FILE_NAME_SIZE,p_Inp->encoderLog);
    if(!strcmp(linesInLog,"  Frame          POC  Pic#   QP    SnrY     SnrU     SnrV   Y:U:V Time(ms)\n")){
      fgets(linesInLog,FILE_NAME_SIZE,p_Inp->encoderLog);
      readyInLog=1;
    }
  }

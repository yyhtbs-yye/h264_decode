 ...
 *		- Yves Dhondt                      <yves.dhondt@ugent.be>
 *
 *************************************************************************************
 */
...
void concealed_pic_report(VideoParameters * p_Vid, StorablePicture * concealed_pic); // YD
...
void conceal_lost_frames(VideoParameters *p_Vid)
{
  ...
  while (CurrFrameNum != UnusedShortTermFrameNum)
  {
    ...
    //gvwallen-begin
    p_Vid->PrevPicOrderCntLsb = picture->poc%(1<<(p_Vid->active_sps->log2_max_pic_order_cnt_lsb_minus4+4));
    p_Vid->PrevPicOrderCntMsb = picture->poc/(1<<(p_Vid->active_sps->log2_max_pic_order_cnt_lsb_minus4+4))*(1<<(p_Vid->active_sps->log2_max_pic_order_cnt_lsb_minus4+4));
    //gvwallen-end
    ...
    if(p_Vid->IDR_concealment_flag == 1)
    {
	  ...
	  p_Vid->IDR_concealment_flag = 0; // YD: if this is not set, all frames following IDR, will be concealed as IDR as well
    }
	...
	// YD: begin
	// print statistics
    concealed_pic_report(p_Vid, picture);

    // update stats
    ++(p_Vid->number); // it is a reference picture
    ++(p_Vid->snr->frame_ctr);
	// YD: end
...
void conceal_one_lost_frame(VideoParameters *p_Vid,int logPOC,int logFrameNum)
{
  int CurrFrameNum;
  int UnusedShortTermFrameNum;
  StorablePicture *picture = NULL;
  ...
  if(p_Vid->IDR_concealment_flag == 1)
  {
    ...
    UnusedShortTermFrameNum = logFrameNum;
  ...
  //while (CurrFrameNum != UnusedShortTermFrameNum)
  {
    ...
    picture->top_poc=logPOC;
    picture->bottom_poc=logPOC;
    picture->frame_poc=logPOC;
    picture->poc=logPOC;
    p_Vid->last_ref_pic_poc = logPOC;
...
void conceal_one_lost_non_ref_frame(VideoParameters *p_Vid,int logPOC,int logFrameNum)
{
  DecodedPictureBuffer *p_Dpb = p_Vid->p_Dpb;
  int missing_poc;
  StorablePicture *concealed_pic;
  FrameStore concealed_fs;
  
  
  
  // one time allocation
  if (p_Dpb->last_output_picture == NULL)
  {
    p_Dpb->last_output_picture = alloc_storable_picture(p_Vid, FRAME, p_Vid->width, p_Vid->height, p_Vid->width_cr, p_Vid->height_cr);
  }

  //if (p_Dpb->last_output_poc >= 0)
  {
    // if there is a gap between the poc of the frame last send to the output and
    // the one which will be send next, then there are missing non reference frames
    //while (p_Dpb->last_output_poc + p_Vid->poc_gap < picture->poc)
    {
      // calculate the poc of the missing non ref frame
      missing_poc = logPOC;

      p_Vid->earlier_missing_poc = missing_poc;
      p_Vid->frame_to_conceal = logFrameNum;
      p_Vid->conceal_slice_type = B_SLICE;

      // allocate memory for the frame to conceal
      concealed_pic = alloc_storable_picture(p_Vid, FRAME, p_Vid->width, p_Vid->height, p_Vid->width_cr, p_Vid->height_cr);

      concealed_pic->top_poc= missing_poc;
      concealed_pic->bottom_poc= missing_poc;
      concealed_pic->frame_poc= missing_poc;
      concealed_pic->poc= missing_poc;
      concealed_pic->pic_num = logFrameNum;
      concealed_pic->used_for_reference = 0;

      // do a simple frame copy (this could be replaced by better methods)
      //copy_to_conceal(p_Dpb->last_output_picture, concealed_pic, p_Vid);
      copy_prev_pic_to_concealed_pic(concealed_pic, p_Vid);
      //store_picture_in_dpb(p_Vid, concealed_pic);

	    // YD: begin
	    // print statistics
        concealed_pic_report(p_Vid, concealed_pic);


      p_Dpb->last_output_picture->concealed_pic = 1;

      // print report
      //concealed_pic_report(p_Vid, concealed_pic);

      // wrap the copy in a FrameStore
      concealed_fs.frame = concealed_pic;//p_Dpb->last_output_picture;
      concealed_fs.is_output = 0;
      concealed_fs.is_reference = 0;
      concealed_fs.is_used = 3;

      // write it out
      write_stored_frame(p_Vid, &concealed_fs, p_Vid->p_out);

      // adjust the last output poc (didn't the above function call already do that?)
      p_Dpb->last_output_poc = missing_poc;
    }
  }

  // store a copy of the frame which is going to be send to the output
  //   note 1: reuse of the memory allocated for 'last_output_picture' can only
  //           be done if the picture does not change in resolution (assumed here)
  //   note 2: in case of an alternative concealment technique, one might want
  //           to copy more info for use by the concealment algorithm
  //copy_to_conceal(picture, p_Dpb->last_output_picture, p_Vid);

  // not sure if the following is needed (with the exception of pic_num)
/*  p_Dpb->last_output_picture->top_poc= picture->poc;
  p_Dpb->last_output_picture->bottom_poc= picture->poc;
  p_Dpb->last_output_picture->frame_poc= picture->poc;
  p_Dpb->last_output_picture->poc= picture->poc;
  p_Dpb->last_output_picture->pic_num = picture->pic_num;
 */ //p_Dpb->last_output_picture->used_for_reference = 0;

  p_Vid->p_Dpb->last_output_poc = logPOC;
...
************************************************************************
* \brief
*   Prints a report on a concealed reference picture.
*
* \param concealed_picture
*   The concealed picture to print the report about.
************************************************************************
*/
void concealed_pic_report(VideoParameters * p_Vid, StorablePicture * concealed_picture)
{
  ...
  calculate_frame_no(p_Vid, concealed_picture);

  structure  = concealed_picture->structure;
  slice_type = concealed_picture->slice_type;
  frame_poc  = concealed_picture->frame_poc;
  refpic     = concealed_picture->used_for_reference;
  qp         = concealed_picture->qp;
  pic_num    = concealed_picture->pic_num;
  is_idr     = concealed_picture->idr_flag;
  ...
  if (p_Inp->silent == FALSE)
  {
    ...
    if (p_Vid->p_ref != -1)
    {
      find_snr(p_Vid, concealed_picture, &(p_Vid->p_ref));
      fprintf(stdout,"%05d(%s%5d %5d    -- %8.4f %8.4f %8.4f  %s    conc\n",
      p_Vid->frame_no, p_Vid->cslice_type, frame_poc, pic_num, snr->snr[0], snr->snr[1], snr->snr[2], yuvFormat);
    }
    else
    {
      fprintf(stdout,"%05d(%s%5d %5d    --                             %s    conc\n",
      p_Vid->frame_no, p_Vid->cslice_type, frame_poc, pic_num, yuvFormat);
    }
  }
  else
  {
    fprintf(stdout,"Completed Concealing frame %05d.\r",snr->frame_ctr);
  }
}
...
************************************************************************
* \brief
*   Conceals non reference pictures.
*
* \param picture
*   The next picture which is going to be send to the output.
************************************************************************
*/
void conceal_lost_non_ref_frames(VideoParameters *p_Vid, StorablePicture *picture)
{
  DecodedPictureBuffer *p_Dpb = p_Vid->p_Dpb;
  int missing_poc;
  StorablePicture *concealed_pic;
  ...
  if (p_Dpb->last_output_poc >= 0)
  {
    ...
    {
      // calculate the poc of the missing non ref frame
      missing_poc = p_Dpb->last_output_poc + p_Vid->poc_gap;

      p_Vid->earlier_missing_poc = missing_poc;
      p_Vid->frame_to_conceal = p_Dpb->last_output_picture->frame_num + 1;
      p_Vid->conceal_slice_type = B_SLICE;
      ...
      concealed_pic->frame_poc= missing_poc;
      concealed_pic->poc= missing_poc;
      concealed_pic->pic_num = p_Dpb->last_output_picture->pic_num;
      concealed_pic->used_for_reference = 0;
      ...
      // wrap the copy in a FrameStore
      concealed_fs.frame = p_Dpb->last_output_picture;
      concealed_fs.is_output = 0;
      concealed_fs.is_reference = 0;
      concealed_fs.is_used = 3;

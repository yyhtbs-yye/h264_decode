...
#include "sei.h"
#include "input.h"
#include "erc_api.h" // YD: added to conceal lost non reference frames
...
static void write_out_picture(VideoParameters *p_Vid, StorablePicture *p, int p_out)
{
  ...
  // YD: begin error concealment for non-reference frames
  /*if (p_Vid->conceal_mode != 0)
  {
    conceal_lost_non_ref_frames(p_Vid, p);
  }*/
  // YD: end error concealment for non-reference frames

  // YD: this is the only certain place to indicate the last_output_poc
  // NS: only do this when concealment is activated
  if(p_Vid->p_Inp->conceal_mode != 0)
  {
    p_Vid->p_Dpb->last_output_poc = p->poc;
  }
